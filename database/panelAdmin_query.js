...
export const consultas_panelAdmin = {
  ...
  getProcedimientos:
    "SELECT DATE_FORMAT(FROM_UNIXTIME(unix_timestamp(tra_pro.`fecha_tratam_proced`)),'%Y/%m/%d %H:%i:%s') as 'Fecha', concat_ws(' - ',proced.`cod_proced`,proced.`nom_proced`) as 'Procedimiento', ing.`monto_ingreso` as 'Ingreso', concat_ws(' - ',tratam.`codigoCIE`, cie.`nombre_enfermedad`) as 'Diagnostico', concat_ws(' - ',tipo_tratam.`tratam_tipoTratam` ,DATE_FORMAT(FROM_UNIXTIME(unix_timestamp(tratam.`fecha_tratam`)),'%Y/%m/%d %H:%i')) as 'Tratamiento', concat_ws(' - ',tcons.`tipo_tipoConsul`,con.`mot_consulta`, DATE_FORMAT(FROM_UNIXTIME(unix_timestamp(con.`fecha_consulta`)),'%Y-%m-%d')) as 'Consulta', concat_ws(' ', pac.`priNom_paciente`, pac.`segNom_paciente` ,pac.`priApe_paciente`, pac.`segApe_paciente`) as 'Paciente', tra_pro.`id_tratam_proced`,proced.`id_proced`,tratam.`id_tratam`, con.`id_consulta`,pac.`id_paciente`,ing.`id_ingreso` FROM `tratamiento_procedimiento_tbl` as tra_pro INNER JOIN `procedimiento_tbl` as proced ON tra_pro.`id_proced` = proced.`id_proced` INNER JOIN `tratamiento_tbl` as tratam ON tra_pro.`id_tratam` = tratam.`id_tratam` INNER JOIN `consulta_tbl` as con ON tratam.`id_consulta` = con.`id_consulta` INNER JOIN `tipoConsulta_tbl` as tcons ON con.`id_tipoConsul` = tcons.`id_tipoConsul` INNER JOIN `paciente_tbl` as pac ON con.`id_paciente`= pac.`id_paciente` LEFT JOIN `tipoTratamiento_tbl` as tipo_tratam ON tratam.`id_tipoTratam` = tipo_tratam.`id_tipoTratam` LEFT JOIN `ingreso_tbl` as ing ON tra_pro.`id_tratam_proced` = ing.`id_tratam_proced` LEFT JOIN `enfermedades_cie10` as cie ON tratam.`codigoCIE` = cie.`codigoCIE`",
